...
const Cast=(props)=>{
  let showButton=props.showButton?"d-none":"btn btn-outline-success";
  let textDark=props.textDark?"text-dark":" "
    let ele=[]
    ...
    ele.push(
  <li className={`media mt-3 ${textDark}`}  key={element.id}>
...
<button data-id={element.id} onClick={e=>{props.callback(null,null,e.target.dataset.id)}} className={showButton}>Find More Movies With {element.name}</button>
